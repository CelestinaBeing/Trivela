...
use soroban_sdk::{symbol_short, vec, Address, Bytes, BytesN, Env, IntoVal, Symbol, Vec as SdkVec};
use trivela_rewards_contract::{Error as RewardsError, RewardsContract, RewardsContractClient};

// ── Helper Functions ─────────────────────────────────────────────────────────

fn setup_rewards(env: &Env) -> (RewardsContractClient, Address) {
...
fn has_event(env: &Env, topic: Symbol) -> bool {
    env.events().all().iter().any(|e| {
        e.topics.get(0).map(|t| t == &topic.into_val(env)).unwrap_or(false)
    })
}
...
fn test_credit_for_campaign_with_multiplier() {
    ...
    assert!(has_event(&env, symbol_short!("multset")));
    ...
    assert!(has_event(&env, symbol_short!("credit")));
}
...
fn test_batch_credit() {
    ...
    // Should emit credit event for each user
    let events = env.events().all();
    let credit_events: Vec<_> = events.iter()
        .filter(|e| e.topics.get(0).map(|t| t == &symbol_short!("credit").into_val(&env)).unwrap_or(false))
        .collect();
...
fn test_admin_transfer() {
    ...
    assert!(has_event(&env, symbol_short!("transfer")));
}
...
fn test_pause_credit() {
    ...
    assert!(has_event(&env, symbol_short!("pscredit")));
...
fn test_pause_claim() {
    ...
    assert!(has_event(&env, symbol_short!("psclaim")));
...
fn test_pause_redeem() {
    ...
    assert!(has_event(&env, symbol_short!("psredeem")));
...
fn test_max_credit_per_call() {
    ...
    assert!(has_event(&env, symbol_short!("mxcredit")));
...
fn test_rate_limit() {
    ...
    assert!(has_event(&env, symbol_short!("ratlset")));
...
fn test_snapshots() {
    ...
    assert!(has_event(&env, symbol_short!("snapshot")));
...
fn test_vesting_flow() {
    ...
    assert!(has_event(&env, symbol_short!("vcredit")));
    assert_eq!(vest_id, 0);
...
fn test_redemption_flow() {
    ...
    // assert!(has_event(&env, symbol_short!("redeem")));
...
fn test_referral_flow() {
    ...
    assert!(has_event(&env, symbol_short!("refcfg")));
    ...
    assert!(has_event(&env, symbol_short!("refbonus")));
    assert!(has_event(&env, symbol_short!("credit")));
...
fn test_sep41_approve_and_transfer_from() {
    ...
    assert!(has_event(&env, symbol_short!("approve")));
...
fn test_sep41_burn() {
    ...
    assert!(has_event(&env, symbol_short!("burn")));
}
